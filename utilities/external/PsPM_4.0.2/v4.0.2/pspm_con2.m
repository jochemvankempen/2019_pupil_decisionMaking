...
% pspm_con2 is a function to set up second level contrasts. Currently, one- 
...
% $Id: pspm_con2.m 450 2017-07-03 15:17:02Z tmoser $
% $Rev: 450 $ 
...
% check outfile
if exist(outfile)==2 && ~options.overwrite
    overwrite=menu(sprintf('Output file (%s) already exists. Overwrite?', outfile), 'yes', 'no');
...
if sample==1
   ...
   for n=1:size(t.beta{1}, 2)
        sd = sqrt(((size(t.beta{1}, 1)-1) * (std(t.beta{1}(:,n))).^2 + (size(t.beta{2}, 1)-1) * (std(t.beta{2}(:,n))).^2) ./ t.df(n));
...
if numel(connames)~=numel(con) && ~strcmpi(conoption, 'number')
    errmsg=sprintf('Number of contrast names (%d) does not match number of contrasts (%d) - contrasts will be numbered instead', numel(connames), numel(con));
    warning(errmsg);
    conoption = 'number';
end;
